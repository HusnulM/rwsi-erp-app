...
class Bom extends Controller {
    ...
    public function createnewversion($bomid){
        $check = $this->model('Home_model')->checkUsermenu('bom', 'Update');
        if ($check){
            $data['title'] = 'Create New BOM Version';
            $data['menu']  = 'Create New BOM Version';

            // Wajib di semua route ke view--------------------------------------------
            $data['setting']  = $this->model('Setting_model')->getgensetting();    //--
            $data['appmenu']  = $this->model('Home_model')->getUsermenu();         //--
            //-------------------------------------------------------------------------   
            $latestversion = $this->model('Bom_model')->getLatestVersion($bomid);
            // echo json_encode($latestversion['latestversion']);

            $data['bomhead']    = $this->model('Bom_model')->bomHeader($bomid);
            $data['bomdata']    = $this->model('Bom_model')->bomVersionDetail($bomid, $latestversion['latestversion']);
            $data['cust']       = $this->model('Customer_model')->customerList();
            $data['_cust']      = $this->model('Customer_model')->getCustomerById($data['bomhead']['cust_id']);
            $data['bomdtl']     = json_encode($data['bomdata']);
            $data['bomid']      = $bomid;
            $data['bomnewversion'] = ($latestversion['latestversion']*1)+1;
            // $data['bomversion'] = $this->model('Bom_model')->bomVersionList($bomid);
            // echo json_encode($data['bomdata']);

            $this->view('templates/header_a', $data);
            $this->view('bom/createnewversion', $data);
            $this->view('templates/footer_a');
        }else{
            $this->view('templates/401');
        }        
    ...
    public function loadbombyversion($bomid, $bomversion){
        $data = $this->model('Bom_model')->bomVersionDetail($bomid, $bomversion);
    ...
    public function bomcalculation($bomid,$qty,$version){
        $data = $this->model('Bom_model')->bomcalculation($bomid,$qty,$version);
        echo json_encode($data);
    }
    
    public function convertbomtopr($bomid,$qty,$version){
    ...
    public function savenewversion(){
        if( $this->model('Bom_model')->createNewBomVersion($_POST) > 0 ) {
			$return = array(
                "msgtype" => "1",
			...
			$return = array(
                "msgtype" => "2",
                "message" => "Error Create New BOM Version!"
            );
            echo json_encode($return);
			exit;	
        }
    ...
    public function exportbom($bomid,$inputqty,$strdate,$enddate,$qtycct,$version){
        $bomheader = $this->model('Bom_model')->bomHeader($bomid);
        $bomdetail = $this->model('Bom_model')->bomcalculation($bomid,$inputqty,$version);
		...
		$excel->setActiveSheetIndex(0)->setCellValue('C10', "Description"); // 
		$excel->setActiveSheetIndex(0)->setCellValue('D10', "Quantity"); // 
		$excel->setActiveSheetIndex(0)->setCellValue('E10', "Total Requirement"); // 
		$excel->setActiveSheetIndex(0)->setCellValue('F10', "Unit"); // 
		...
		foreach($bomdetail as $i => $h){ // Ambil semua data dari hasil eksekusi $sql
            ...
            $excel->setActiveSheetIndex(0)->setCellValue('E'.$numrow, $h['Total']);
