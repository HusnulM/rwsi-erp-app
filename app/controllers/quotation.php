...
// 			$data['quotation']  = $this->model('Quotation_model')->listquotation();
	...
	public function bomversoinlist($bomid){
		...
		$data['data'] = $this->model('Quotation_model')->bomVersionList($bomid);
		echo json_encode($data);
	}

    public function previewquotation($bomid,$profit=null,$persen,$version){
		if($profit === null){
			$profit = 0;
		}else{
		...
		$data['bomitem']     = $this->model('Quotation_model')->bomVersionDetail($bomid,$version);
    ...
    public function generatequotation($bomid,$qdate,$profit=null,$persen,$topprice,$version){
