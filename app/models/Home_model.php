...
// 			$this->db->query("SELECT * FROM v_user_menu WHERE username='$user' and type='parent'");
// 			return $this->db->resultSet();
// 		}
// 	}

// 	public function getUserMenuSetting(){
// 		if( isset($_SESSION['usr']) ){
// 			$user = $_SESSION['usr_erp']['user'];
// 			$this->db->query("SELECT * FROM v_user_menu WHERE username='$user' and type='parent' and grouping = 'setting'");
// 			return $this->db->resultSet();
// 		}
// 	}
    ...
    public function getUsermenu(){
		if( isset($_SESSION['usr_erp']) ){
			$user = $_SESSION['usr_erp']['user'];
			$this->db->query("SELECT distinct * FROM v_user_menu WHERE username='$user' and type='parent'");
			return $this->db->resultSet();
		}
	}
	...
	public function getUserMenuSetting(){
		if( isset($_SESSION['usr_erp']) ){
			$user = $_SESSION['usr_erp']['user'];
			$this->db->query("SELECT distinct * FROM v_user_menu WHERE username='$user' and type='parent' and grouping = 'setting'");
			return $this->db->resultSet();
		}
	}
	...
	public function getuserbyreffid($reffid){
		$this->db->query("SELECT *, fGetJabatan(jabatan) as 'jbtn', fGetDepartment(department) as 'department' FROM t_user WHERE reffid=:reffid");
		$this->db->bind('reffid',$reffid);
		$this->db->execute();
		return $this->db->single();
	}
	...
	public function login($data){
		$this->db->query("SELECT *, fGetJabatan(jabatan) as 'jbtn', fGetDepartment(department) as 'department' FROM t_user WHERE username=:username");
		$this->db->bind('username',$data['username']);
		$this->db->execute();
		$result = $this->db->rowCount();
