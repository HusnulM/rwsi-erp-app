...
class Quotation_model{
    ...
    public function bomVersionDetail($bomid, $version){
        $this->db->query("SELECT * FROM v_quotation WHERE bomid='$bomid' and bom_version='$version'");
        return $this->db->resultSet(); 
    }

    public function bomVersionList($bomid){
        $this->db->query("SELECT distinct bom_version, createdon from t_bom02 WHERE bomid='$bomid'");
        return $this->db->resultSet(); 
    }
