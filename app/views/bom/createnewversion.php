<section class="content">
                ...
                <div class="card">
                    <div class="header">
                        ...
                        <ul class="header-dropdown m-r--5">        
                            <button type="button" id="btn-change" class="btn btn-success waves-effect">
                                Change
                            </button>                        
                            <a href="<?= BASEURL; ?>/bom" class="btn btn-danger waves-effect">
                                Cancel
                            </a>
                        </ul>
                    </div>
                    <div class="body">
                        <div class="row clearfix">
                            ...
                            <div class="col-lg-2 col-md-6 col-sm-12">
                                <div class="form-group">
                                    <div class="form-line">
                                        <label for="bom_version">BOM New Version</label>
                                        <input type="number" name="bom_version" id="bomVersion" class="form-control" style="text-align:right;" readonly value="<?= $data['bomnewversion']; ?>">
                                    </div>
                                </div>
                            </div>
                        </div>                            
    ...
    <script>
        ...
        $(function(){
            ...
            $('#bomVersion').on('change', function(){
                // loadbomitem();
                var bomVersion = this.value;
                // loadBomVersionDetail(bomid, bomVersion);
            });

            function loadBomVersionDetail(_bomid, _version){
                // loadbombyversion
                $('#tbl-pr-body').html('');
                $.ajax({
                    url:base_url+'/bom/loadbombyversion/'+_bomid+'/'+_version,
                    method:'get',
                    dataType:'JSON',
                    contentType: false,
                    cache: false,
                    ...
                    for(var i=0; i<dataBom.length; i++){
                        var bomqty = '0';
                        bomqty = dataBom[i].quantity.replaceAll('.00','');
                        ...
                        html = `
                            <tr counter="`+ count +`" id="tr`+ count +`">
                                ...
                                <td> 
                                    <input type="text" name="itm_material[]" counter="`+count+`" id="material`+count+`" class="form-control materialCode" readonly style="width:100%;" required="true" value="`+ dataBom[i].component +`" />
                                </td>
                                <td> 
                                    <input type="text" name="itm_matdesc[]" counter="`+count+`" id="matdesc`+count+`" class="form-control" readonly style="width:100%;" value="`+ dataBom[i].matdesc +`"/>
                                </td>
                                <td> 
                                ...
                                <td> 
                                    <input type="text" name="itm_unit[]" counter="`+count+`" id="unit`+count+`" class="form-control" readonly style="width:65px; readOnly" required="true" value="`+ dataBom[i].unit +`"/>
                                </td>
            ...
            $('#form-pr-data').on('submit', function(event){
                ...
                $.ajax({
                    url:base_url+'/bom/savenewversion',
                    method:'post',
                    data:formData,
                    dataType:'JSON',
                    contentType: false,
